...
def check_collision():
    ...
    for brick in bricks:
        if canvas.coords(
                ball)[2] >= canvas.coords(brick)[0] and canvas.coords(ball)[0] <= canvas.coords(brick)[2] and canvas.coords(ball)[3] >= canvas.coords(brick)[1] and canvas.coords(ball)[1] <= canvas.coords(brick)[3]:
            canvas.delete(brick)
            bricks.remove(brick)
            ball_dy *= -1
            break
    ...
    if canvas.coords(
            ball)[0] <= 0 or canvas.coords(ball)[2] >= CANVAS_WIDTH:
        ball_dx *= -1
    ...
    if canvas.coords(
            ball)[3] >= canvas.coords(paddle)[1] and canvas.coords(ball)[2] >= canvas.coords(paddle)[0] and canvas.coords(ball)[0] <= canvas.coords(paddle)[2]:
        ball_dy *= -1
...
def update_ball():
    ...
    ball_x, ball_y = canvas.coords(
        ball)[0] + BALL_RADIUS, canvas.coords(ball)[1] + BALL_RADIUS
    check_collision()
